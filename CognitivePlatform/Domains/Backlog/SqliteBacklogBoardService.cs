...
public sealed class SqliteBacklogBoardService : IBacklogBoardService
{
    ...
    public async Task<BacklogStoryDto> CreateStoryAsync(CreateBacklogStoryRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var displayId = request.DisplayId.HasNoValue() ? $"STORY-{nextNumber}" : request.DisplayId!.Trim();
    ...
    public async Task<BacklogStoryDto> ArchiveStoryAsync(Guid storyId, ArchiveBacklogStoryRequest request, CancellationToken cancellationToken = default)
    {
        ...
        EnsureRevision(existing, request.ExpectedRevision);
        if (!existing.ColumnKey.EqualsIgnoreCase("done"))
    ...
    public async Task<BacklogReferenceDto> CreateReferenceAsync(string referenceType, CreateBacklogReferenceRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (request.Key.HasNoValue() || request.Name.HasNoValue()) throw new BacklogValidationException("A key and name are required.");
        await using var connection = await OpenAsync(cancellationToken);
        await using var command = connection.CreateCommand();
    ...
    public async Task<BacklogReferenceDto> UpdateReferenceAsync(string referenceType, string key, UpdateBacklogReferenceRequest request, CancellationToken cancellationToken = default)
    {
        await EnsureInitializedAsync(cancellationToken);
        var normalizedType = NormalizeReferenceType(referenceType);
        if (key.HasNoValue() || request.Name.HasNoValue()) throw new BacklogValidationException("A key and name are required.");
        await using var connection = await OpenAsync(cancellationToken);
    ...
    {
        await EnsureInitializedAsync(cancellationToken);
        if (request.Description.HasNoValue()) throw new BacklogValidationException("Intake description is required.");
    ...
    private static IReadOnlyDictionary<string, string> NormalizeProperties(IReadOnlyDictionary<string, string>? properties) => properties?.Where(entry => entry.Key.HasValue()).ToDictionary(entry => entry.Key.Trim(), entry => entry.Value?.Trim() ?? string.Empty, StringComparer.OrdinalIgnoreCase) ?? new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
    private static void ValidateStory(CreateBacklogStoryRequest request) { if (new[] { request.ProjectKey, request.AreaKey, request.ItemTypeKey, request.ColumnKey, request.Title, request.Description }.Any(value => value.HasNoValue())) throw new BacklogValidationException("Project, area, type, column, title, and description are required."); if (request.Priority is < 1 or > 99) throw new BacklogValidationException("Priority must be between 1 and 99."); }
                                                   ...
                                                   , CancellationToken cancellationToken)
    {
        ...
        return story is not null && story.ColumnKey.EqualsIgnoreCase(column)
    ...
    private static async Task RepairBuiltInColumnNamesAsync(SqliteConnection connection, CancellationToken cancellationToken) { await using var command = connection.CreateCommand(); command.CommandText = "UPDATE BacklogReferences SET Name = 'Ready' WHERE ReferenceType = 'column' AND Key = 'planned' AND Name = 'Planned';"; await command.ExecuteNonQueryAsync(cancellationToken); }

    private static readonly (string Type, string Key, string Name, int SortOrder)[] Defaults = [ ("project", "cognitive-platform", "Cognitive Platform", 10), ("area", "unassigned", "Unassigned", 10), ("column", "backlog", "Backlog", 10), ("column", "planned", "Ready", 20), ("column", "in-progress", "In Progress", 30), ("column", "done", "Done", 40), ("type", "story", "Story", 10), ("type", "bug", "Bug", 20), ("type", "enhancement", "Enhancement", 30), ("type", "technical-debt", "Technical Debt", 40), ("type", "ux", "UX / UI", 50), ("property", "release", "Release", 10), ("property", "owner", "Owner", 20), ("property", "estimate", "Estimate", 30), ("property", "dependency", "Dependency", 40), ("property", "customer-impact", "Customer Impact", 50) ];
