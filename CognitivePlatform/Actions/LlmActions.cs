...
public static class LlmActions
{
    ...
    private static readonly AsyncLocal<ConversationContext?> _contextLocal = new();
    ...
    private static readonly AsyncLocal<LlmModelCatalog?> _catalogLocal = new();
    ...
    private static readonly AsyncLocal<LlmProviderDefaults?> _providerDefaultsLocal = new();
    ...
    public static void SetProviderDefaults( LlmProviderDefaults defaults ) => _providerDefaults = defaults;

    [NaturalLanguageAction(Description = "Sets the LLM model to use for the remainder of this session. "
    ...
    {
        if (_context is null) return "Session context is not available. SetContext was not called.";
        if (model.HasNoValue()) return "Please provide a model name. Say 'list models' to see what is available.";
    ...
    [NaturalLanguageAction(Description = "Lists the LLM models that are available and usable in the current session."
                         , Examples =
                           [
                                   "List models"
    ...
    {
        if (_catalog is null) return "Model catalog is not available.";

        if (_catalog.AvailableModels.Count == 0) return "No models have been probed yet. The catalog is empty.";
    ...
    [NaturalLanguageAction(Description = "Switches the LLM provider for the remainder of this session "
    ...
    {
        if (_context is null) return "Session context is not available. SetContext was not called.";
        if (provider.HasNoValue()) return "Please provide a provider name. Say 'list providers' to see what is available.";

        var trimmed = provider.Trim();

        if (Enum.TryParse<LlmProvider>(trimmed
                                     , ignoreCase: true
                                     , out var parsed).Not())
        {
            var available = string.Join(", "
                                      , Enum.GetNames<LlmProvider>());
    ...
    [NaturalLanguageAction(Description = "Lists the LLM providers known to the system, marks the active session "
    ...
    {
        if (_catalog is null
         || _catalog.AvailableModels.Count == 0)
