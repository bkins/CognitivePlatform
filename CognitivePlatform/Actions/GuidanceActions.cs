...
public static class GuidanceActions
{
    ...
    [NaturalLanguageAction(
        Description = "Explains how a specific feature area works, including available operations, example phrases, and relationships to other features."
      , Examples    =
        [
                "Tell me about Tasks"
    ...
    public static string GetDomainGuidance(string domainName)
    {
        ...
        return BuildGuidanceResponse(domainName, allActions);
    }
    ...
    /// </summary>
    internal static string BuildGuidanceResponse(
        string                      domainName
      , IEnumerable<ActionMetadata> allActions)
    {
        ...
        var domainActions = allActions
            .Where(action => string.Equals(
                       action.Domain?.Name ?? action.Category
                     , canonical
    ...
    private static string? ResolveDomainAlias(string input)
    {
        ...
        if (KnownDomainAliases.TryGetValue(normalized, out var canonical))
            return canonical;

        // Partial prefix match — "task management" → "Tasks"
        foreach (var kvp in KnownDomainAliases)
        {
    ...
    private static readonly Dictionary<string, string> DomainDescriptions =
        new(StringComparer.OrdinalIgnoreCase)
        {
                {
                    "Tasks"
                  , "Tasks are your to-do list with built-in Eisenhower-matrix priority awareness. "
              ...
              , {
                    "Journal"
                  , "The Journal is an append-only log of your thoughts, reflections, and daily experiences. "
              ...
              , {
                    "Daily"
                  , "Daily Records give your day structure: open it in the morning with a plan and tasks, "
              ...
              , {
                    "Calendar"
                  , "Calendar connects to your Google Calendar account and lets you view events, check free time, "
              ...
              , {
                    "Knowledge"
                  , "The Knowledge Inbox surfaces patterns and insights generated by the Insight Engine "
              ...
              , {
                    "Identity"
                  , "Your Identity profile is a structured, curated model of who you are: "
              ...
              , {
                    "System"
                  , "System actions let you inspect the platform (version, environment, health), "
                  + "switch LLM models and providers, list available capabilities, and get guidance on any feature."
                }
              , {
                    "Personality"
                  , "Personalities control how the assistant communicates — tone, style, and warmth. "
              ...
              , {
                    "Personas"
                  , "Personas are named synthetic people you create and have conversations with. "
                  + "Each persona builds a memory over the conversation and can be loaded back in later sessions."
                }
              , {
                    "PersonaEngine"
                  , "The Persona Engine manages which persona is active in the current conversation. "
    ...
    private static readonly Dictionary<string, string> DomainRelationships =
        new(StringComparer.OrdinalIgnoreCase)
        {
                {
                    "Tasks"
                  , "Tasks appear in your **Daily Plan** when you open the day. "
              ...
              , {
                    "Journal"
                  , "The **Insight Engine** reads your recent journal entries to detect mood trends and stress patterns. "
              ...
              , {
                    "Daily"
                  , "Tasks added in a Daily Plan appear in your **Tasks** list. "
                  + "Closing the day rolls unfinished tasks forward. "
                  + "The **Insight Engine** can factor daily-record patterns into its analysis."
                }
              , {
                    "Calendar"
                  , "Calendar is standalone today — events are not yet automatically linked to Tasks or Daily Records. "
                  + "Free-time queries make it easy to find slots for scheduling tasks."
                }
              , {
                    "Knowledge"
                  , "Insights are generated from **Journal**, **Task**, and **Daily Record** data. "
                  + "Your **Identity** profile feeds into the insight context so suggestions are personalized."
                }
              , {
                    "Identity"
                  , "The Identity profile is used by the **Insight Engine** to personalize suggestions. "
                  + "Derived insights from your **Journal** history can be confirmed and promoted into your profile."
                }
              , {
                    "Personality"
                  , "Personality is independent of domain — it affects every response the assistant gives. "
