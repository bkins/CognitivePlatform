...
| ENH-06 | Add other free AI APIs as fallback LLM providers | CP API / LlmClientFactory | **Done 2026-04-21** — OpenRouter and Cerebras added via shared `OpenAiCompatibleLlmClient`. Both added to `LlmProvider` enum, settings classes, factory switch. Activate via `LlmClient:Provider = "OpenRouter"` or `"Cerebras"` and supply the API key in user-secrets. Further providers (GitHub, Mistral, etc.) deferred. |
| ENH-07 | Set default LLM model via natural-language prompt (session-scoped) | CP API / LlmActions | **Done 2026-04-21** — `LlmActions.SetModel(model)` validates against the model catalog and stores the preference as `session_model` in `ConversationContext.Metadata`; persists across turns for the session. `ListModels()` returns all probed models with usability status. Provider switching deferred — see DEFERRED.md. |
